...
spreadsheet = client.open("ReimbursementRequests")  # Your spreadsheet name
...
def status():
    if request.method == 'POST':
        ...
        if status:
            return f'Status of request {request_id}: {status}'
        else:
...
def save_submission(general_info, items):
    ...
    row_data.append("Pending")  # Initial status
    worksheet.append_row(row_data)
    request_id = worksheet.row_count  # Use row count as a unique request ID
    return request_id
...
def check_status(request_id):
    # Check the status of the request in the Google Sheet
    try:
        request_id = int(request_id)
        row = worksheet.row_values(request_id)
        if row:
            return row[-1]  # Return the status from the last column
    except:
        return None
